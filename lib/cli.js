...
const npmGet = util.promisify((uri, params, cb) =>
  npm.get(uri, params, (err, ...results) => cb(err, results))
);
const npmFetch = util.promisify((uri, params, cb) =>
  npm.fetch(uri, params, (err, data) => cb(err, data))
);
...
module.exports = async function (argv) {
  ...
  for (const moduleName of moduleNames) {
    // const [{ versions: fromVersions }] = await npmGet(
    //   `${from.url}/${moduleName}`,
    //   {
    //     auth: from.auth,
    //     timeout: 3000,
    //   }
    // );
    const fromVersions = (await npmFetch2.json(
      `${from.url}/${moduleName}`,
      scoped
    )).versions;
    ...
    var toVersions;
    try {
      // const [{ versions: temp }] = await npmGet(`${to.url}/${moduleName}`, {
      //   auth: to.auth,
      //   timeout: 3000,
      // });
      toVersions = (await npmFetch2.json(
        `${to.url}/${moduleName}`,
        scoped
      )).versions;
    } catch (error) {
    ...
    for (const semver in fromVersions) {
      const oldMetadata = fromVersions[semver];
      ...
      const { dist } = oldMetadata;
      const remoteTarball = await npmFetch2(dist.tarball, scoped);
      try {
        const res = await npmPublish(`${to.url}`, {
          auth: to.auth,
